...
export class ZamTokenBSC extends ethereum.SmartContract {
  static bind(address: Address): ZamTokenBSC {
    return new ZamTokenBSC("ZamTokenBSC", address);
  }
  ...
  mintTo(amount: BigInt, recipient: Address): boolean {
    let result = super.call("mintTo", "mintTo(uint256,address):(bool)", [
      ethereum.Value.fromUnsignedBigInt(amount),
  ...
  try_mintTo(amount: BigInt, recipient: Address): ethereum.CallResult<boolean> {
    let result = super.tryCall("mintTo", "mintTo(uint256,address):(bool)", [
      ethereum.Value.fromUnsignedBigInt(amount),
      ethereum.Value.fromAddress(recipient)
    ]);
    if (result.reverted) {
  ...
  mintable(): boolean {
    let result = super.call("mintable", "mintable():(bool)", []);

    return result[0].toBoolean();
  }

  try_mintable(): ethereum.CallResult<boolean> {
    let result = super.tryCall("mintable", "mintable():(bool)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
